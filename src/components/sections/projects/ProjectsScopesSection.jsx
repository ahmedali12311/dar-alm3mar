...
export default function ProjectsScopesSection() {
  return (
    <section className="relative bg-[#f7f4ef] py-24 sm:py-32 overflow-hidden">
      {/* Top accent line */}
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#915025]/20 to-transparent" />

      {/* Large background text */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none whitespace-nowrap text-[20vw] font-black uppercase leading-none text-[#915025]/[0.03]"
      >
        SCOPE
      </div>
          ...
          {workScopes.map((item, i) => (
            ...
            >
              ...
              <div className="flex items-center justify-end mb-2">
                <span className="text-2xl text-[#915025]/40 group-hover:text-[#915025]/70 transition-colors duration-300">
                  {icons[i % icons.length]}
                </span>
              </div>

              {/* Divider */}
              <div className="h-px w-full bg-slate-100 group-hover:bg-[#915025]/15 transition-colors duration-300" />

              {/* Content */}
              <div>
                <h3 className="font-['Cairo'] text-lg font-bold text-slate-900 mb-3 group-hover:text-[#915025] transition-colors duration-300">
                  {item.title}
                </h3>
              ...
              <div className="mt-auto flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="text-[10px] font-black uppercase tracking-[0.35em] text-[#915025]">اعرف أكثر</span>
                <span className="text-[#915025]">←</span>
              </div>
            </article>
          ))}
        </div>
